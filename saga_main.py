...
flags.DEFINE_boolean('dp_saga', True, 'If True, train with DP-sag. If False, train with vanilla SAG.')
...
def main(argv):
    ...
    noise_multiplier = FLAGS.noise_multiplier if FLAGS.dp_saga else -1
    clip = FLAGS.l2_norm_clip if FLAGS.dp_saga else -1
    ...
    name = 'dpSAGA_'+str(FLAGS.warmup)
    log_dir = os.path.join(FLAGS.dir, FLAGS.data, name,
                           utils.get_fn(EasyDict(batch=batch),
                                        EasyDict(dpsgd=FLAGS.dp_saga, restart=FLAGS.restart, completion=FLAGS.tree_completion, noise=noise_multiplier, clip=clip, mb=1),
                                        [EasyDict({'lr': lr}),
    ...
    optimizer = SAGAOptimizer(model.parameters(), mean_grad, std, num_batches, device, shapes, momentum=FLAGS.momentum,
    ...
    def get_cumm_noise(effi_noise):
        if FLAGS.dp_saga == False or noise_multiplier == 0:
